...
contract('SolnSquareVerifier', accounts => {
    ...
    describe('Test suite: addSolution', () => {
        ...
        before(async() => { 
            ...
            contractInstance = await solnSquareContractDefinition.new(verifier.address, {from: accounts[0]});
        }); 
        ...
        it('should allow to add a solution with correct values and emit event', async() => {
            ...
            truffleAssert.eventEmitted(tx, 'SolutionAdded', (ev) => {
                return expect(Number(ev.solIndex)).to.equal(0) && expect(ev.solAddress).to.equal(accounts[0]);
            });
        });
        ...
        it('should allow to add more new solutions with correct values and emit event', async() => {
            ...
            truffleAssert.eventEmitted(tx, 'SolutionAdded', (ev) => {
                return expect(Number(ev.solIndex)).to.equal(1) && expect(ev.solAddress).to.equal(accounts[0]);
            });
        });
