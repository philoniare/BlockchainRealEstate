...
contract('ERC721MintableComplete', accounts => {
    ...
    describe('Test suite: Ownable inherited', function () {
        before(async() => { 
            contractInstance = await contractDefinition.new({from: account_one});
        ...
        it('should NOT allow unauthorized address to transfer ownership', async() => {
            await expectToRevert(contractInstance.transferOwnership(account_two, {from: account_two}), 'Caller is not owner');
        });
        ...
        it('should allow to transfer ownership and emit event', async() => {
            ...
            truffleAssert.eventEmitted(tx, 'OwnershipTransferred', (ev) => {
                return expect(ev.currentOwner).to.deep.equal(currentOwner) && expect(ev.newOwner).to.equal(account_two);
            });
        ...
        it('should fail when minting when caller is not contract owner', async() => { 
            await expectToRevert(contractInstance.mint(account_two, 12, {from: account_one}), 'Caller is not owner');
        });
    ...
    describe('Test suite: Pausable inherited', function () {
        ...
        it('should NOT allow unauthorized address to pause the contract', async() => {
            await expectToRevert(contractInstance.setPaused(true, {from: account_two}), 'Caller is not owner');
        });

        it('should NOT allow unauthorized address to unpause the contract', async() => {
            await expectToRevert(contractInstance.setPaused(false, {from: account_two}), 'Caller is not owner');
        });

        it('should allow owner to pause the contract and emit event', async() => {
            let tx = await contractInstance.setPaused(true, {from: account_one});
        ...
        it('should NOT allow owner to pause the contract when contract is already paused', async() => {
            await expectToRevert(contractInstance.setPaused(true, {from: account_one}), 'New pause state should be different');
        });

        it('should allow owner to unpause the contract and emit event', async() => {
            let tr = await contractInstance.setPaused(false, {from: account_one});
        ...
        it('should NOT allow owner to unpause the contract when contract is already unpaused', async() => {
            await expectToRevert(contractInstance.setPaused(false, {from: account_one}), 'New pause state should be different');
        });

        it('should fail when minting when contract is paused', async() => { 
            await contractInstance.setPaused(true, {from: account_one});
            await expectToRevert(contractInstance.mint(account_two, 12, {from: account_one}), 'Contract is paused');
        });
    ...
    describe('Test suite: ERC721MintableComplete', () => {
       const tokensIds = [11, 22, 33, 44, 55, 66, 77, 88, 99, 101];
        ...
        it('should not mint to an invalid address 0x0', async() => {
            await expectToRevert(contractInstance.mint(zeroAddress, 211), "Given address is not valid");
        });
        ...
        // token uri should beP complete i.e: https://s3-us-west-2.amazonaws.com/udacity-blockchain/capstone/1
        ...
        it('should NOT allow to transfer to an invalid address 0x0', async() => {
            await expectToRevert(contractInstance.transferFrom(accounts[8], zeroAddress, tokensIds[7], {from: accounts[8]}), "Recipient address is not valid");
        });
    });
