...
contract Ownable {
    ...
    function transferOwnership(address newOwner) public onlyOwner {
        require(newOwner == address(newOwner), "New owner address is invalid");
...
contract Pausable is Ownable {
    ...
    function setPaused(bool _isPaused) public onlyOwner {
        require(_isPaused != _paused, "New pause state should be different");
        _paused = _isPaused;
        if(_paused == true) {
...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public {
        require(to != ownerOf(tokenId), "Recipient is already the owner");
        require(msg.sender == ownerOf(tokenId) || isApprovedForAll(ownerOf(tokenId), to), "Approval not authorized");
    ...
    function _mint(address to, uint256 tokenId) internal {
        require(_tokenOwner[tokenId] == address(0), "tokenId exists");
    ...
    function _transferFrom(address from, address to, uint256 tokenId) internal {
        require(from == ownerOf(tokenId), "Unauthorized transfer");
...
contract ERC721MintableComplete is ERC721Metadata {
    constructor() ERC721Metadata("HouseLord", "HLD", "https://s3-us-west-2.amazonaws.com/udacity-blockchain/capstone/") public {
